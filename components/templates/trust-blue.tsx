// Trust Blue - Bank-Approved Financial Grade
import React from 'react'
import { InvoiceData, CurrencyDisplay, TemplateLogo } from './template-utils'

export const TrustBlue: React.FC<{ data: InvoiceData }> = ({ data }) => {
  return (
    <div className="w-full h-full bg-white p-16 font-sans" style={{ fontFamily: 'system-ui, sans-serif' }}>
      <div className="flex justify-between items-start mb-12">
        <div>
          ...
          <h1 className="text-2xl font-bold text-sky-900">{data.company.name}</h1>
        ...
        <div className="text-right">
          <h2 className="text-4xl font-bold text-sky-700">INVOICE</h2>
      ...
      <div className="mb-12 p-6 bg-sky-50 rounded-lg border-l-4 border-sky-600">
        <p className="text-xs uppercase tracking-wider text-sky-800 font-bold mb-2">Bill To</p>
      ...
      <div className="flex justify-end mb-12">
        <div className="w-80">
          ...
          <div className="flex justify-between py-4 text-xl font-bold bg-sky-700 text-white px-4 mt-2">
            <span>TOTAL</span>
      ...
      {data.notes && <div className="mb-6 p-4 bg-sky-50 rounded-lg"><p className="text-xs uppercase tracking-wider text-sky-800 mb-1">Notes</p><p className="text-sm text-gray-700">{data.notes}</p></div>}
      {data.terms && <div className="p-4 bg-sky-50 rounded-lg"><p className="text-xs uppercase tracking-wider text-sky-800 mb-1">Terms</p><p className="text-sm text-gray-700">{data.terms}</p></div>}
    </div>
  )
}
