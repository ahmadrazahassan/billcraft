// Neural Network - AI Machine Learning Aesthetic
import React from 'react'
import { InvoiceData, CurrencyDisplay, TemplateLogo } from './template-utils'

export const NeuralNetwork: React.FC<{ data: InvoiceData }> = ({ data }) => {
  return (
    <div className="w-full h-full bg-white p-16 font-mono" style={{ fontFamily: 'JetBrains Mono, monospace' }}>
      <div className="flex justify-between items-start mb-12 pb-6 border-b-4 border-indigo-600">
        <div>
      ...
      <div className="mb-12 p-6 bg-indigo-50 rounded-xl border-2 border-indigo-300">
        <p className="text-xs uppercase tracking-widest text-indigo-800 font-bold mb-2">// BILL TO</p>
      ...
      <div className="flex justify-end mb-12">
        <div className="w-80">
          ...
          <div className="flex justify-between py-4 text-xl font-bold bg-indigo-700 text-white px-4 rounded-lg mt-2">
            <span>TOTAL</span>
      ...
      {data.notes && <div className="mb-6 p-4 bg-indigo-50 rounded-xl"><p className="text-xs uppercase tracking-widest text-indigo-800 mb-1">// NOTES</p><p className="text-sm text-indigo-800">{data.notes}</p></div>}
      {data.terms && <div className="p-4 bg-indigo-50 rounded-xl"><p className="text-xs uppercase tracking-widest text-indigo-800 mb-1">// TERMS</p><p className="text-sm text-indigo-800">{data.terms}</p></div>}
    </div>
  )
}
