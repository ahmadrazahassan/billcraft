// Boardroom Gray - Enterprise Meeting Room Ready
import React from 'react'
import { InvoiceData, CurrencyDisplay, TemplateLogo } from './template-utils'

export const BoardroomGray: React.FC<{ data: InvoiceData }> = ({ data }) => {
  return (
    <div className="w-full h-full bg-white p-16 font-sans" style={{ fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div className="mb-12 p-6 bg-gray-50 rounded-lg border-l-4 border-gray-600">
        <p className="text-xs uppercase tracking-wider text-gray-700 font-bold mb-2">Bill To</p>
      ...
      <div className="flex justify-end mb-12">
        <div className="w-80">
          ...
          <div className="flex justify-between py-4 text-xl font-bold bg-gray-700 text-white px-4 mt-2">
            <span>TOTAL</span>
      ...
      {data.notes && <div className="mb-6 p-4 bg-gray-50 rounded-lg"><p className="text-xs uppercase tracking-wider text-gray-700 mb-1">Notes</p><p className="text-sm text-gray-700">{data.notes}</p></div>}
      {data.terms && <div className="p-4 bg-gray-50 rounded-lg"><p className="text-xs uppercase tracking-wider text-gray-700 mb-1">Terms</p><p className="text-sm text-gray-700">{data.terms}</p></div>}
    </div>
  )
}
