// Electric Purple - Vibrant Creative Agency
import React from 'react'
import { InvoiceData, CurrencyDisplay, TemplateLogo } from './template-utils'

export const ElectricPurple: React.FC<{ data: InvoiceData }> = ({ data }) => {
  return (
    <div className="w-full h-full bg-white p-16 font-sans" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      <div className="flex justify-between items-start mb-12">
        <div>
          ...
          <h1 className="text-2xl font-bold text-purple-900">{data.company.name}</h1>
        ...
        <div className="text-right">
          <h2 className="text-4xl font-black text-purple-600">INVOICE</h2>
      ...
      <div className="mb-12 p-6 bg-purple-50 rounded-2xl border-2 border-purple-300">
        <p className="text-xs uppercase tracking-wider text-purple-700 font-bold mb-2">Bill To</p>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-purple-600 text-white">
              <th className="text-left py-3 px-4 text-xs uppercase tracking-wider font-bold rounded-tl-lg">Description</th>
              ...
              <th className="text-right py-3 px-4 text-xs uppercase tracking-wider font-bold w-32 rounded-tr-lg">Amount</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div className="flex justify-end mb-12">
        <div className="w-80">
          ...
          <div className="flex justify-between py-4 text-xl font-black bg-purple-600 text-white px-4 rounded-xl mt-2">
            <span>TOTAL</span>
      ...
      {data.notes && <div className="mb-6 p-4 bg-purple-50 rounded-xl"><p className="text-xs uppercase tracking-wider text-purple-700 mb-1">Notes</p><p className="text-sm text-gray-700">{data.notes}</p></div>}
      {data.terms && <div className="p-4 bg-purple-50 rounded-xl"><p className="text-xs uppercase tracking-wider text-purple-700 mb-1">Terms</p><p className="text-sm text-gray-700">{data.terms}</p></div>}
    </div>
  )
}
