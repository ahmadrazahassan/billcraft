// Hologram Blue - 3D Holographic Sci-Fi
import React from 'react'
import { InvoiceData, CurrencyDisplay, TemplateLogo } from './template-utils'

export const HologramBlue: React.FC<{ data: InvoiceData }> = ({ data }) => {
  return (
    <div className="w-full h-full bg-gradient-to-br from-blue-50 to-slate-100 p-16 font-sans" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      <div className="bg-white/90 backdrop-blur-sm p-12 rounded-3xl shadow-2xl border-2 border-blue-300">
        <div className="flex justify-between items-start mb-12">
          ...
          <div className="text-right">
            ...
            <p className="text-sm font-bold text-blue-900 mt-2">#{data.invoiceNumber}</p>
            <p className="text-sm text-blue-700 mt-1">{data.date}</p>
            <p className="text-sm text-blue-700">Due: {data.dueDate}</p>
          </div>
        </div>

        <div className="mb-12 p-6 bg-blue-50/80 backdrop-blur-sm rounded-2xl border-2 border-blue-300">
          <p className="text-xs uppercase tracking-wider text-blue-800 font-bold mb-2">Bill To</p>
          <p className="font-bold text-blue-900">{data.client.name}</p>
            ...
            <tbody>
              {data.items.map((item, index) => (
                <tr key={index} className="border-b border-blue-200">
                  ...
                  <td className="py-3 px-4 text-sm text-right text-blue-900 font-bold"><CurrencyDisplay amount={item.amount} currency={data.currency} /></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex justify-end mb-12">
          <div className="w-80">
            <div className="flex justify-between py-2 text-sm border-b border-blue-200">
            ...
            <div className="flex justify-between py-4 text-xl font-bold bg-blue-700 text-white px-4 rounded-xl mt-2">
              <span>TOTAL</span>
        ...
        {data.notes && <div className="mb-6 p-4 bg-blue-50/80 backdrop-blur-sm rounded-xl"><p className="text-xs uppercase tracking-wider text-blue-800 mb-1">Notes</p><p className="text-sm text-blue-800">{data.notes}</p></div>}
        {data.terms && <div className="p-4 bg-blue-50/80 backdrop-blur-sm rounded-xl"><p className="text-xs uppercase tracking-wider text-blue-800 mb-1">Terms</p><p className="text-sm text-blue-800">{data.terms}</p></div>}
      </div>
    </div>
  )
}
