// Executive Navy - C-Suite Fortune 500
import React from 'react'
import { InvoiceData, CurrencyDisplay, TemplateLogo } from './template-utils'

export const ExecutiveNavy: React.FC<{ data: InvoiceData }> = ({ data }) => {
  return (
    <div className="w-full h-full bg-white p-16 font-sans" style={{ fontFamily: 'system-ui, sans-serif' }}>
      <div className="flex justify-between items-start mb-12 pb-6 border-b-4 border-blue-900">
        <div>
        ...
        <div className="text-right">
          <h2 className="text-4xl font-bold text-blue-900">INVOICE</h2>
          <p className="text-sm font-bold text-gray-900 mt-2">#{data.invoiceNumber}</p>
          <p className="text-sm text-gray-700 mt-1">{data.date}</p>
          <p className="text-sm text-gray-700">Due: {data.dueDate}</p>
        </div>
      </div>

      <div className="mb-12 p-6 bg-blue-50 rounded-lg border-l-4 border-blue-900">
        <p className="text-xs uppercase tracking-wider text-blue-900 font-bold mb-2">Bill To</p>
      ...
      <div className="flex justify-end mb-12">
        <div className="w-80">
          ...
          <div className="flex justify-between py-4 text-xl font-bold bg-blue-900 text-white px-4 mt-2">
            <span>TOTAL</span>
      ...
      {data.notes && <div className="mb-6 p-4 bg-blue-50 rounded-lg"><p className="text-xs uppercase tracking-wider text-blue-900 mb-1">Notes</p><p className="text-sm text-gray-700">{data.notes}</p></div>}
      {data.terms && <div className="p-4 bg-blue-50 rounded-lg"><p className="text-xs uppercase tracking-wider text-blue-900 mb-1">Terms</p><p className="text-sm text-gray-700">{data.terms}</p></div>}
    </div>
  )
}
