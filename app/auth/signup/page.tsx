...
import { buildLoginURL, parseURLParams } from '@/lib/url-builder'
...
// SignupFormContent component that uses useSearchParams
function SignupFormContent() {
  ...
  const onSubmit = async (data: SignupForm) => {
    ...
    try {
      ...
      // Store plan selection in user metadata/database
      // In production, you would save this to Firestore/Supabase:
      // await saveUserPlanSelection({
      //   userId: user.uid,
      //   plan: selectedPlan,
      ...
      success({
        ...
        description: `Your account has been created with ${selectedPlan} plan. Enjoy your ${trialDays}-day free trial!`,
      })
      router.replace('/dashboard')
    } catch (error: any) {
      error({
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Store plan selection for Google sign-in users
      // Same as above - save to database in production
        ...
        >
            ...
            <div className="text-center mb-8">
              ...
              {selectedPlan !== 'free' && (
                <motion.div
                  ...
                  className="inline-flex items-center space-x-2 px-4 py-2 rounded-full bg-primary/10 text-primary font-semibold text-xs border border-primary/20 mb-4"
                >
                  <Crown className="h-3.5 w-3.5" />
                  <span className="capitalize">{selectedPlan} Plan Selected</span>
                </motion.div>
              )}
              ...
              >
                ...
                <p className="text-muted-foreground text-sm">
                  {hasTrial ? `Start your ${trialDays}-day free trial` : 'Get started'} • No credit card required
                </p>
              </motion.div>
          </div>
          ...
          {/* Footer */}
            ...
            >
              ...
              <Link
                href={buildLoginURL({ source: 'signup_page', ref: 'signup_to_login_link' })}
              ...
              <p className="text-muted-foreground text-xs mt-4">
                ...
                <Link href="/terms?source=signup_page&ref=terms_link" className="text-primary font-medium hover:underline transition-all duration-300">Terms</Link>
                {' '}and{' '}
                <Link href="/privacy?source=signup_page&ref=privacy_link" className="text-primary font-medium hover:underline transition-all duration-300">Privacy Policy</Link>
              </p>
            </motion.div>
        </motion.div>
      </div>
...
// Main SignupPage component that wraps SignupFormContent in Suspense
